...
export default function QuickAddClientModal({ onClose, onCreated }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[999] p-4 animate-in fade-in duration-200">
            <div className={`rounded-2xl border w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col ${isDark ? 'bg-gray-900 border-white/10' : 'bg-white border-gray-200'}`}>

                {/* Header */}
                <div className={`flex items-center justify-between px-6 py-4 border-b shrink-0 ${isDark ? 'border-white/[0.06] bg-gray-900/95 backdrop-blur' : 'border-gray-100 bg-white/95 backdrop-blur'}`}>
